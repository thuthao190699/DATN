<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PhepNoiLien.aspx.cs" Inherits="DATN.PhepNoiLien" %>

<!DOCTYPE html>

<html>
<body>
    <h2>Phép nối liền.</h2>
    <p>Mệnh đề nối liền của 2 mệnh đề P, Q được ký hiệu bởi P ^ Q (đọc là P và Q).</p> 
    <p>Chân trị của P ^ Q là 1 nếu cả P lẫn Q đều có chân trị 1. Trong các trường hợp khác, P ^ Q có chân trị 0.</p>
    <h4>Quy tắc chân trị</h4>
    <img src="../Images/QuyTacChanTri_b.jpg" alt="Girl in a jacket" style="width:400px;height:300px;">
    <hr />
    <h3>Ví dụ:</h3>
    <p>Mệnh đề “Hôm nay trời đẹp và trận bóng đá sẽ hấp dẫn” được xem như là một mệnh đề đúng nếu cả 2 điều kiện “trời đẹp” và “trận bòng đá hấp dẫn” đều xảy ra.</p>
    <p>Ngược lại nếu chỉ có 1 trong 2 điều kiện trên đúng hoặc cả 2 đều sai thì mệnh đề là một mệnh đề sai.</p>
    <hr />
    <h3>Đề xuất tìm kiếm:</h3>
    <p><a href="MenhDeSoCap.aspx" target ="_blank">Mệnh đề sơ cấp</a></p>
    <p><a href="MenhDePhucHop.aspx" target ="_blank">Mệnh đề phức hợp</a></p>
    <p><a href="PhepPhuDinh.aspx" target ="_blank">Phép nối liền</a></p>
    <p><a href="PhepNoiRoi.aspx" target ="_blank">Phép nối rời</a></p>
    <p><a href="PhepKeoTheo.aspx" target ="_blank">Phép kéo theo</a></p>
    <p><a href="PhepKeoTheoHaiChieu.aspx" target ="_blank">Phép kéo theo 2 chiều</a></p>
</body>
</html>
